# asterisk-push-notify-mobile — explicação linha a linha
...
- `loadConfig()`: carrega as configurações do arquivo `/etc/asterisk-push-notify-mobile/push.env`
...
Lê um arquivo no formato `CHAVE=VALOR` (como o `/etc/asterisk-push-notify-mobile/push.env`) e
